function [ f , x , iter , xxbar , ffbar,fcount ] = global_RBF_TRM ( myfun , xbar...
    , fbar , ind , xstar, ls, method, deg, gamma, useg )
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
if (useg==1)
    ...
    for i=1:length(ls)
        ...
        while (norm(g)>tol && iter2<itbnd && norm(s)>tol2)
            qpval=g*s+0.5*s'*H*s;
            [f_neworig,g_new]=feval(myfun,(x+s)');% changed due to python only takes 1*N array
            fcount = fcount +1;
    ...
    for i=1:length(ls)
        ...
        while (norm(g)>tol && iter2<itbnd && norm(s)>tol2)
            ...
            %disp(count)
            qpval=g'*s+0.5*s'*H*s;
            f_neworig=feval(myfun,(x+s)');% changed due to python only takes 1*N array
            fcount = fcount +1;
            %disp('f_neworig')
            %disp(f_neworig)
            f_new=f_neworig+lambda*norm(x+s-xstar)^2;
