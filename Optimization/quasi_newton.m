...
tol=1e-7; % changed from 1e-4 to 1e-2 due to the computing time
itbnd=500*1e20;
n=length(x0);
H=eye(n);
x=x0;
...
while(norm(g)>tol && abs(utils(end) - utils(end - 1)) > 1e-8 && iter<itbnd && fcount<1e6) % changed fcount < 1e3 to 1e2 due to the computing time 
